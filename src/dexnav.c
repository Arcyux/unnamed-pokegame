...
static void Task_DexNavSearch(u8 taskId)
{
    ...
    // only require moving slowly for hidden mons
    ...
    // *loud incorrect buzzer noise* only hidden pokemon anywhere move around
    if (sDexNavSearchDataPtr->isHiddenMon//(sDexNavSearchDataPtr->environment == ENCOUNTER_TYPE_WATER || GetCurrentMapType() == MAP_TYPE_UNDERGROUND)
...
static void CreateDexNavWildMon(u16 species, u8 potential, u8 level, u8 abilityNum, u16 item, u16* moves)
{
    ...
    if (DexNavTryMakeShinyMon())
        FlagSet(FLAG_DEXNAV_CREATE_SHINY); // just easier this way
    ...
    FlagClear(FLAG_DEXNAV_CREATE_SHINY);
}
...
{
    if (gSaveBlock1Ptr->dexNavSearchLevels[dexNum] < 255)
