...
#define StringBufferSize 32
static EWRAM_DATA u8 (* sListMenuStringsBuffer)[StringBufferSize] = NULL;
...
static void CreateTMCaseListMenuBuffers(void)
{
    ...
    sListMenuStringsBuffer = Alloc(sTMCaseDynamicResources->numTMs * StringBufferSize);
}
...
static void InitTMCaseListMenuItems(void)
{
    ...
    gMultiuseListMenuTemplate.windowId = WIN_ITEM_LIST;
...
static void PrintListMenuCursorAt_WithColorIdx(u8 a0, u8 a1)
{
    if (a1 == 0xFF)
    {
        FillWindowPixelRect(WIN_ITEM_LIST, 0, 0, a0, GetFontAttribute(2, 0), GetFontAttribute(2, 1));
        CopyWindowToVram(WIN_ITEM_LIST, 2);
    }
    else
    {
        AddTextPrinterParameterized_ColorByIndex(WIN_ITEM_LIST, 2, gText_SelectorArrow2, 0, a0, 0, 0, 0, a1);
    }
}
...
static void Task_SelectTMAction_FromFieldBag(u8 taskId)
{
    ...
    TMCase_SetWindowBorder(WIN_SELECTED_TM); // context window border style
    strbuf = Alloc(256);
    GetTMNumberAndMoveString(strbuf, gSpecialVar_ItemId);
    StringAppend(strbuf, gText_Var1IsSelected + 2); // +2 skips over the stringvar
    AddTextPrinterParameterized_ColorByIndex(WIN_SELECTED_TM, 2, strbuf, 0, 2, 1, 0, 0, 4);
    Free(strbuf);

    //show HM icon
    if (gSpecialVar_ItemId >= ITEM_HM01)
    {
        PlaceHMTileInWindow(WIN_SELECTED_TM, 0, 2);
        CopyWindowToVram(WIN_SELECTED_TM, 2);
    }
...
static void TMHMContextMenuAction_Use(u8 taskId)
{
    ...
    ClearStdWindowAndFrameToTransparent(WIN_SELECTED_TM, 0);
    ClearWindowTilemap(WIN_SELECTED_TM);
...
static void InitWindowTemplatesAndPals(void)
{
    ...
    LoadMessageBoxGfx(WIN_ITEM_LIST, 0x64, BG_PLTT_ID(11));
    LoadUserWindowBorderGfx(WIN_ITEM_LIST, 0x78, BG_PLTT_ID(13));
    LoadUserWindowBorderGfx(WIN_ITEM_LIST, 0x78, BG_PLTT_ID(14)); // load a second time because first pal is edited
    LoadPalette(gTMCaseMainWindowPalette, BG_PLTT_ID(15), 0x20);
    LoadPalette(gTMCaseMainWindowPalette, BG_PLTT_ID(10), 0x20);
    LoadPalette(sPal3Override, BG_PLTT_ID(15) + 6, 0x04);
    LoadPalette(sPal3Override, BG_PLTT_ID(13) + 6, 0x04);
    ...
    PutWindowTilemap(WIN_ITEM_LIST);
    PutWindowTilemap(WIN_DESCRIPTION);
    PutWindowTilemap(WIN_TMCASE_NAME);
    PutWindowTilemap(WIN_TMHM_INFO_ICONS);
    PutWindowTilemap(WIN_TMHM_INFO);
    PutWindowTilemap(WIN_TMHM_TYPE);
    PutWindowTilemap(WIN_TMHM_CAT);
...
static void PrintStringTMCaseOnWindow3(void)
{
    ...
    AddTextPrinterParameterized3(WIN_TMCASE_NAME, 1, distance / 2, 1, sTMCaseTextColors[0], 0, gText_TMCase);
}
...
void LoadMonIconPalettesTinted(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(gMonIconPaletteTable); i++)
    {
        ...
        CpuCopy32(&gPlttBufferUnfaded[OBJ_PLTT_ID(i)], &gPlttBufferUnfaded[OBJ_PLTT_ID(7 + i)], PLTT_SIZE_4BPP);
        TintPalette_GrayScale2(&gPlttBufferUnfaded[OBJ_PLTT_ID(7 + i)], 16);
    }
}
