...
bool8 ScrCmd_objectfaceplayer(struct ScriptContext* ctx)
{
    u16 localId = VarGet(ScriptReadHalfword(ctx));
    
    ObjectEventTurnByLocalIdAndMap(localId, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup, GetOppositeDirection(GetPlayerFacingDirection()));
    return FALSE;
}
